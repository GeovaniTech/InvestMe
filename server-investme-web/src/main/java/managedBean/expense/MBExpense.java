...
@Named(MBExpense.MANAGED_BEAN_NAME)
@ViewScoped
public class MBExpense extends AbstractFilterMBean<TOTransaction, TOFilterTransaction> {
	
	private static final long serialVersionUID = 8184287297629627293L;
	public static final String MANAGED_BEAN_NAME = "MBExpense";
	
	private List<TOCategory> categories;
	private List<TOPayment> payments;
	private TOTransaction expense;
	...
	public void clearFilters() {
		...
		this.getFilter().setType("expense");
	}

	@Override
	public void onRowSelect(SelectEvent<TOTransaction> event) {
		this.getMBTransactionInfo().editExpense(event.getObject());
	}
	
	public void initExpense() {
		this.getMBTransactionInfo().initExpense();
	}
	...
	public TOTransaction getExpense() {
		return expense;
	}

	public void setExpense(TOTransaction expense) {
